...
export async function request(accountId, action, zone, params = {}) {
  ...
  query.signature = crypto.createHmac("sha256", decryptSecret(row.secret_ciphertext)).update(`GET\n/iaas/\n${queryValues(query)}`).digest("base64");
  const response = await fetch(`https://api.qingcloud.com/iaas/?${new URLSearchParams(query)}`);
  const data = await response.json().catch(() => ({}));
  const requestParams = { zone, ...params };
  if (!response.ok || Number(data?.ret_code || 0) !== 0) { const message = data?.message || data?.ret_message || `青云 ${response.status}`; writeApiLog(accountId, "api.qingcloud.com", action, requestParams, data, "失败", message); throw new Error(message); }
  writeApiLog(accountId, "api.qingcloud.com", action, requestParams, data, "成功");
  return data;
}
...
export async function buckets(accountId) {
  ...
  const items = (data.buckets || []).map((bucket) => ({ Name: bucket.name, BucketName: bucket.name, Location: bucket.location || "", CreationDate: bucket.created || "", StorageClass: "STANDARD", Acl: "private", ExtranetEndpoint: Array.isArray(bucket.urls) ? bucket.urls[0] || "-" : "-", IntranetEndpoint: "-", _region_id: bucket.location || "" }));
...
export async function resources(accountId, type, zones = ["pek3a"]) {
  if (type === "oss") {
  ...
  for (const zone of zones) {
    try {
      const data = await request(accountId, action, zone, { limit: 100 });
      items.push(...(data[key] || []).map((item) => normalize(item, zone)));
    } catch (error) { errors.push(`${zone}: ${error.message}`); }
  }
  return { resource_type: type, items, errors, fetched_at: Date.now() };
}

export async function verify(accountId, zones = ["pek3a"]) {
  await request(accountId, "DescribeInstances", zones[0], { limit: 1 });
