...
export type PanelDiskInfo = { path: string; detail: string; percent: number | null };
...
export function panelLoadText(value: unknown): string {
  const values = Array.isArray(value) ? value.slice(0, 3) : [panelMetricField(value, ["one"]), panelMetricField(value, ["five"]), panelMetricField(value, ["fifteen"])];
...
export function panelDiskItems(value: unknown): PanelDiskInfo[] {
  ...
  return disks.map((disk) => {
    ...
    return { path: String(path || "-"), detail: used === undefined || total === undefined ? "-" : `${formatPanelStorage(used)} / ${formatPanelStorage(total)}${percent === null ? "" : ` (${formatPanelNumber(percent)}%)`}`, percent };
...
export function panelNetworkInfo(value: unknown): { up: string; down: string } {
  ...
  return { up: rate(panelMetricNumber(directUp) ?? legacyRate(["up", "upload"])), down: rate(panelMetricNumber(directDown) ?? legacyRate(["down", "download"])) };
}
